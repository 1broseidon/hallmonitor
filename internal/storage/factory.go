...
const (
	...
	// BackendPostgres uses PostgreSQL/TimescaleDB for storage
	BackendPostgres BackendType = "postgres"
	// BackendInfluxDB uses InfluxDB for storage
...
func NewStore(cfg *config.StorageConfig, logger *logging.Logger) (ResultStore, error) {
	...
	case BackendPostgres:
		...
		store, err := NewPostgresStore(connString, logger)
		if err != nil {
			return nil, err
		}

		// Start retention cleanup if configured
		if cfg.Postgres.RetentionDays > 0 {
			store.StartRetentionCleanup(cfg.Postgres.RetentionDays)
		}
